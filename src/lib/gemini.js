...
Adj személyre szabott, konkrét pénzügyi tanácsot magyarul. Legyél barátságos és érthető — ne beszélj könyvelői szakzsargonban. Ha az adó vagy jog területén végleges döntésről van szó, zárd a válaszodat pontosan ezzel a mondattal: "Egyeztesd könyvelőddel."`;
  ...
  try {
    ...
    // DEMO VÁLASZOK AZ ESETLEGES API HIBA/KORLÁT ESETÉRE
    const lowerMessage = message.toLowerCase();
    if (lowerMessage.includes('alkalmazott') || lowerMessage.includes('felvenni')) {
      return "Egy új alkalmazott felvétele jelentős többletterhet róhat a cégre. Ha 400.000 Ft bruttó bért feltételezünk, az a szociális hozzájárulási adóval (13%) együtt havi ~452.000 Ft bérköltséget jelent a munkáltatónak. Javaslom, használd a Cashflow 'Mi van ha' szimulátorát a pontos likviditási hatás elemzéséhez!";
    }
    if (lowerMessage.includes('áfa') || lowerMessage.includes('afa') || lowerMessage.includes('afa-kör')) {
      return "Jelenlegi szabályok szerint az alanyi adómentesség (AAM) értékhatára évi 12 millió forint. Ezt nagyon fontos nyomon követni, mert ha túlléped, akkor automatikusan ÁFA-t kell fizetned a számláid után! A megadott bevételeid alapján ezt ősszel megközelítheted. Kérlek, egyeztess erről könyvelőddel.";
    }
    if (lowerMessage.includes('kata') || lowerMessage.includes('átalányadó') || lowerMessage.includes('adózás')) {
      return "A bevételeid növekedésével érdemes lehet felülvizsgálni a jelenlegi adózási formádat. Kérlek navigálj az 'Adózási Tanácsadó' menüpontba, ahol a csúszkák segítségével forintra pontosan láthatod a különböző formák közötti különbségeket. Döntés előtt egyeztesd könyvelőddel!";
    }
    if (lowerMessage.includes('cashflow') || lowerMessage.includes('egyenleg') || lowerMessage.includes('likvid')) {
      return "A cashflow modul előrejelzése alapján a nyári hónapokban mérsékelt bevételkiesés, majd szeptemberben nagy fellendülés várható. Nagyon figyelj az októberi iparűzési adó befizetésre, mert az komoly, 1.1 millió forintos terhet jelent, amire már most javasolt elkezdeni a tartalékolást.";
    }

    return "Ez egy kiváló kérdés! Az iparágadat és a jelenlegi árbevételedet figyelembe véve azt javaslom, fókuszálj a stabil ügyfeleid megtartására, és a meglévő bevételek optimalizálására. Kérlek, próbálj meg konkrét kulcsszavakra rákérdezni (pl. alkalmazott, áfa, adózás, cashflow), hogy részletesebb választ adhassak!";
  }
}
